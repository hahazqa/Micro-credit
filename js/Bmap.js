window.BMAP_AUTHENTIC_KEY = "C1SEj9RpkRvGwTeBy2Ptr6NQ3UUzQ674";
(function() {
	function aa(a) {
		throw a;
	}
	...
	x.extend(Ja.prototype, {
		...
		ik: function(a, b, c, d) {
			...
			if (c = c && d ? f.Jm(c, this.Vb) : this.pc)
				if (this.pc = new H(c.lng + a * e, c.lat - b * e), (a = f.qh(this.pc, this.Vb)) && g) this.ef = a
		},
		Fg: function(a, b) {
		...
		Ub: function() {
			var a = this.fa() >= this.K.LF && this.ta() === Ka && 18 >= this.fa(),
				b = q;
			try {
		...
		(function(a, b, c) {
			...
			if (!C || !C.Ho) {
				var A = b.all && a.attachEvent,
					B = C && C.sE || +new Date,
					D = a.T2 || (+new Date).toString(36) + Math.random().toString(36).substr(2, 3),
					E = 0,
					J = {},
					O = function(a) {
						var b = arguments,
	...
	x.object.extend(I, {
		...
		bK: function(a) {
			if (a && this.fL()[a])
				for (var a = this.fL()[a], b = 0; b < a.length; b++) this.bK(a[b]), this.Nd.rl[a[b]] || this.jN(a[b])
		},
		jN: function(a) {
	...
	z.Se(function(a) {
		...
		a.addEventListener("addoverlay", function(a) {
			a = a.target;
			if (a instanceof fb) b.va[a.ba] || (b.va[a.ba] = a);
			else {
		...
		a.addEventListener("removeoverlay", function(a) {
			a = a.target;
			if (a instanceof fb) delete b.va[a.ba];
			else
	...
	x.extend(Cc.prototype, {
		...
		Nt: function(a) {
			var b = this.map.ta();
			if (!this.map.Ub() && (a ? this.map.K.i_ = a : a = this.map.K.i_, a))
	...
	x.extend(Ma.prototype, {
		...
		rB: function() {
			if (this.C)
				for (var a = this.C.qx(), b = 0; b < a.length; b++)(a[b] instanceof T || a[b] instanceof qc) && a[b].point && this.va.push(a[b])
		},
		nF: ba("C"),
		Pt: function(a) {
			this.Jf = a || "none"
	...
	var Ud = ac.prototype;
	S(Ud, {
		getPanoramaById: Ud.$o,
		getPanoramaByLocation: Ud.qj,
		getPanoramaByPOIId: Ud.sx
	});
	...
	var Yd = document,
		Zd = Math,
		$d = Yd.createElement("div").style,
		ae;
	a: {
		for (var be = ["t", "webkitT", "MozT", "msT", "OT"], ce, de = 0, ee = be.length; de < ee; de++)
			if (ce = be[de] + "ransform", ce in $d) {
				ae = be[de].substr(0, be[de].length - 1);
				break a
			}
		ae = q
	}
	var fe = ae ? "-" + ae.toLowerCase() + "-" : "",
		he = ge("transform"),
		ie = ge("transitionProperty"),
		je = ge("transitionDuration"),
		ke = ge("transformOrigin"),
		le = ge("transitionTimingFunction"),
		ne = ge("transitionDelay"),
		...
		re = ge("perspective") in $d,
		se = "ontouchstart" in window && !qe,
		te = ae !== q,
		ue = ge("transition") in $d,
		...
		Ce = window.cancelRequestAnimationFrame || window.q4 || window.webkitCancelRequestAnimationFrame || window.mozCancelRequestAnimationFrame || window.oCancelRequestAnimationFrame || window.msCancelRequestAnimationFrame || clearTimeout,
		De = re ? " translateZ(0)" : "";
	...
	Ee.prototype = {
		...
		$u: function(a) {
			if (!(se && 0 !== a.touches.length)) {
				...
				if (b.Wk) d = b.scale * b.op, d = Math.max(b.options.Vk, d), d = Math.min(b.options.Xp, d), b.op = d / b.scale, b.scale = d, b.x = b.$x - b.$x * b.op + b.x, b.y = b.ay - b.ay * b.op + b.y, b.Nb.style[je] = "200ms", b.Nb.style[he] = "translate(" + b.x + "px," + b.y + "px) scale(" + b.scale + ")" + De, b.Wk = q, b.refresh(), b.options.up && b.options.up.call(b, a);
				else {
					if (b.rh) {
	...
	x.extend(Fe.prototype, {
		...
		Dl: function(a) {
			...
			a = this.vH(this.P.Yb(), this.N0); - 1 !== a && this.Jp(a)
		},
		$W: function(a) {
	...
	S(ef, {
		addCopyright: ef.mw,
		removeCopyright: ef.XE,
		getCopyright: ef.om,
		getCopyrightCollection: ef.AD
	});
	S(window, {
