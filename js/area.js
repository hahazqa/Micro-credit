...
function change(v,s_) {
	...
	if (s_==0) {
		...
		for (i = 0; i < v; i++) {
			str += ("_" + (document.getElementById(s_day  [i]).selectedIndex - 1));
		};
		var ss = document.getElementById(s_day[v]);
		if(ss != undefined && ss != null){
			with(ss) {
				length = 0;
				options[0] = new Option(opt0[v], opt0[v]);
				if (v && document.getElementById(s_day  [v - 1]).selectedIndex > 0 || !v) {
					if (dsy.Exists(str)) {
						ar = dsy.Items[str];
						for (i = 0; i < ar.length; i++) {
							options[length] = new Option(ar[i], ar[i]);
		...
		for (i = 0; i < v; i++) {
			str += ("_" + (document.getElementById(day_s_[i]).selectedIndex - 1));
		};
		var ss = document.getElementById(day_s_[v]);
		if(ss != undefined && ss != null){
			with(ss) {
				length = 0;
				options[0] = new Option(opt0[v], opt0[v]);
				if (v && document.getElementById(day_s_[v - 1]).selectedIndex > 0 || !v) {
					if (dsy.Exists(str)) {
						ar = dsy.Items[str];
						for (i = 0; i < ar.length; i++) {
							options[length] = new Option(ar[i], ar[i]);
...
var s_day = ["day_a", "day_b"]; //三个sele
var day_s_ = ["day_s_a", "day_s_b", "day_s_c"]; //三个select的name
var opt0 = ["省份", "城市", "区/县级市"]; //初始值

function _init_area() { //初始化函数
	for (i = 0; i < s.length - 1; i++) {
		document.getElementById(s[i]).onchange = new Function("change(" + (i + 1) + ",0)");
	}
	change(0,0);
}

function _init_area_() { //初始化函数
	for (i = 0; i < s_day.length - 1; i++) {
		document.getElementById(s_day[i]).onchange = new Function("change(" + (i + 1) + ",1)");
	}
	change(0,1);
}

function _init_area__() { //初始化函数
	
	for (i = 0; i < day_s_.length - 1; i++) {
		document.getElementById(day_s_[i]).onchange = new Function("change(" + (i + 1) + ",2)");
	}
	change(0,2);
}